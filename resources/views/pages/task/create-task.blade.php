...
@section('content')
  ...
  <x-common.component-card title="Create Your Task Here">
    ...
    <form action="{{ route('tasks.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6"
      x-data="taskForm()" @submit.prevent="submitForm">
      @csrf
      ...
      <div>
        ...
        <input type="text" name="title" value="{{ old('title') }}" placeholder="Enter task title"
          class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 @error('title') border-red-500 @enderror"
      ...
      <div>
        ...
        <textarea name="description" placeholder="Enter task description..." rows="3"
          class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 @error('description') border-red-500 @enderror"
      ...
      <div x-data="{
        ...
        open: false,
        search: '',
        selected: '{{ old('assigned_user_id') ? $users->firstWhere('id', old('assigned_user_id'))->name ?? "Select User" : "Select User" }}',
        selectedId: '{{ old('assigned_user_id') }}',
        ...
        <!-- Hidden Input -->
        <input type="hidden" name="assigned_user_id" :value="selectedId" required>
      ...
      <div>
        ...
        <div x-data="{ isOptionSelected: {{ old('priority') ? 'true' : 'false' }} }" class="relative z-20 bg-transparent">
      ...
      <div>
        ...
        <div x-data="{ isOptionSelected: {{ old('status') ? 'true' : 'false' }} }" class="relative z-20 bg-transparent">
          <select name="status"
      ...
      <div>
        ...
        <x-form.date-picker id="due_date" name="due_date" placeholder="Date Picker"
          defaultDate="{{ old('due_date', now()->format('Y-m-d')) }}" required />
      ...
      <div>
        ...
        <input type="file" name="attachment" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.webp,.zip"
